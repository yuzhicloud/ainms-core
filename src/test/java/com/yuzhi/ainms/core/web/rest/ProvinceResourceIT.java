...
class ProvinceResourceIT {
  ...
  private static final Integer DEFAULT_PROVINCE_CODE = 1;
  private static final Integer UPDATED_PROVINCE_CODE = 2;
  ...
  void createProvince() throws Exception {
    ...
    assertProvinceUpdatableFieldsEquals(
      returnedProvince,
      getPersistedProvince(returnedProvince)
    );
  }
  ...
  void getAllProvinces() throws Exception {
    ...
    restProvinceMockMvc
      ...
      .andExpect(
        jsonPath("$.[*].id").value(hasItem(province.getId().intValue()))
      )
      .andExpect(
        jsonPath("$.[*].provinceCode").value(hasItem(DEFAULT_PROVINCE_CODE))
      )
      .andExpect(
        jsonPath("$.[*].provinceName").value(hasItem(DEFAULT_PROVINCE_NAME))
      );
  }
  ...
  void putExistingProvince() throws Exception {
    ...
    // Update the province
    Province updatedProvince = provinceRepository
  ...
  void partialUpdateProvinceWithPatch() throws Exception {
    ...
    assertSameRepositoryCount(databaseSizeBeforeUpdate);
    assertProvinceUpdatableFieldsEquals(
      createUpdateProxyForBean(partialUpdatedProvince, province),
      getPersistedProvince(province)
    );
  }
  ...
  void fullUpdateProvinceWithPatch() throws Exception {
    ...
    assertProvinceUpdatableFieldsEquals(
      partialUpdatedProvince,
      getPersistedProvince(partialUpdatedProvince)
    );
  }
  ...
  void deleteProvince() throws Exception {
    ...
    restProvinceMockMvc
      .perform(
        delete(ENTITY_API_URL_ID, province.getId())
          .with(csrf())
  ...
  protected void assertPersistedProvinceToMatchAllProperties(
    Province expectedProvince
  ) {
    assertProvinceAllPropertiesEquals(
      expectedProvince,
      getPersistedProvince(expectedProvince)
    );
  }

  protected void assertPersistedProvinceToMatchUpdatableProperties(
    Province expectedProvince
  ) {
    assertProvinceAllUpdatablePropertiesEquals(
      expectedProvince,
      getPersistedProvince(expectedProvince)
    );
  }
