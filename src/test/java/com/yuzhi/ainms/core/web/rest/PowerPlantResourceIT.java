...
class PowerPlantResourceIT {
  ...
  public static PowerPlant createEntity(EntityManager em) {
    PowerPlant powerPlant = new PowerPlant()
      .powerPlantName(DEFAULT_POWER_PLANT_NAME);
    return powerPlant;
  }
  ...
  public static PowerPlant createUpdatedEntity(EntityManager em) {
    PowerPlant powerPlant = new PowerPlant()
      .powerPlantName(UPDATED_POWER_PLANT_NAME);
    return powerPlant;
  }
  ...
  void createPowerPlant() throws Exception {
    ...
    assertPowerPlantUpdatableFieldsEquals(
      returnedPowerPlant,
      getPersistedPowerPlant(returnedPowerPlant)
    );
  }
  ...
  void getAllPowerPlants() throws Exception {
    ...
    restPowerPlantMockMvc
      ...
      .andExpect(
        jsonPath("$.[*].id").value(hasItem(powerPlant.getId().intValue()))
      )
      .andExpect(
        jsonPath("$.[*].powerPlantName").value(
          hasItem(DEFAULT_POWER_PLANT_NAME)
        )
      );
  }
  ...
  void putExistingPowerPlant() throws Exception {
    ...
    PowerPlant updatedPowerPlant = powerPlantRepository
  ...
  void fullUpdatePowerPlantWithPatch() throws Exception {
    ...
    assertSameRepositoryCount(databaseSizeBeforeUpdate);
    assertPowerPlantUpdatableFieldsEquals(
      partialUpdatedPowerPlant,
      getPersistedPowerPlant(partialUpdatedPowerPlant)
    );
  }
  ...
  void deletePowerPlant() throws Exception {
    ...
    restPowerPlantMockMvc
      .perform(
        delete(ENTITY_API_URL_ID, powerPlant.getId())
          .with(csrf())
  ...
  protected void assertPersistedPowerPlantToMatchAllProperties(
    PowerPlant expectedPowerPlant
  ) {
    assertPowerPlantAllPropertiesEquals(
      expectedPowerPlant,
      getPersistedPowerPlant(expectedPowerPlant)
    );
  }

  protected void assertPersistedPowerPlantToMatchUpdatableProperties(
    PowerPlant expectedPowerPlant
  ) {
    assertPowerPlantAllUpdatablePropertiesEquals(
      expectedPowerPlant,
      getPersistedPowerPlant(expectedPowerPlant)
    );
  }
